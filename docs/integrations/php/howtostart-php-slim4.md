---
title: "dhtmlxGantt with PHP:Slim"
...
:::note
This tutorial uses Slim Framework v4.x. If you need a tutorial for an older version, check the [Slim Framework v3.x](integrations/php/howtostart-php.md) guide.
:::
...
the response is expected to contain Gantt data in the [JSON format](guides/supported-data-formats.md). 
...
- in the handler for that route we read all tasks and links from the database and send them to the client as [JSON](guides/supported-data-formats.md)
...
The dataProcessor will work in the REST mode, which means it will use different HTTP methods for different actions, here is [a complete list of routes](guides/server-side.md#requestresponsedetails).
...
If you want to handle it on a backend, you'll need to switch the [cascade_delete](api/config/cascade_delete.md) config on.
