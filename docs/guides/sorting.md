...
1. By a single click on the header of a column with the enabled [sort](api/config/sort.md) attribute;
2. By the API call (can be called from some event or action, i.e. a button click or a page load) of the [sort](api/method/sort.md) method.
...
To enable sorting in the Gantt chart, set the [sort](api/config/sort.md) property to *true*:
...
## Per column Grid sorting {#percolumngridsorting}
...
[Sort by several properties (fields) with a custom sort function](https://snippet.dhtmlx.com/upu86azw)
