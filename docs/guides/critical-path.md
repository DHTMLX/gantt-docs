...
A task is considered critical if it has no days of slack and any delay would directly affect the project completion date. The detailed explanation of how the logic of calculation of the critical path works is given in the [Critical path logic](#critical-path-logic) section.
...
To show the critical path in the Gantt chart, set the [highlight_critical_path](api/config/highlight_critical_path.md) property to 'true':
...
## Getting free and total slack {#gettingfreeandtotalslack}
...
To get the free slack of a task, make use of the [getFreeSlack](api/method/getfreeslack.md) method. It takes the object of a task as a parameter:
...
To get the total slack of a task, make use of the [getTotalSlack](api/method/gettotalslack.md) method. It takes the object of a task as a parameter as well:
...
- If you return "gantt_critical_task" in the [task_class](api/template/task_class.md) template, the task will be highlighted as a critical one.
- If you return "gantt_critical_link" in the [link_class](api/template/link_class.md) template, the link will be highlighted as a critical one.
...
Optionally, you can enable the [auto_scheduling_use_progress](api/config/auto_scheduling_use_progress.md) config to change this behavior:
