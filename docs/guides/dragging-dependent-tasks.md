---
title: "Dragging Tasks Together with Their Dependent Tasks"
...
To use the auto scheduling functionality, you should enable it using the [gantt.plugins](api/method/plugins.md) method:
...
Then, you need to attach a handler to the [onTaskDrag](api/event/ontaskdrag.md) event. It will be called on each frame of drag and drop, and from here we'll move all linked tasks.
...
Finally, when the user releases the mouse and drag-and-drop is finished, we need to round positions of the child items to scale. We can do it using [onAfterTaskDrag](api/event/onaftertaskdrag.md) event:
...
When the user releases the mouse and drag and drop is finished, we can capture the [onBeforeTaskChanged](api/event/onbeforetaskchanged.md) event,
...
Finally, we capture the [onAfterTaskDrag](api/event/onaftertaskdrag.md) event, which tells that drag-and-drop has been performed. 
