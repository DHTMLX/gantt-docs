---
title: "Extra Elements in Timeline"
sidebar_label: "Extra Elements in Timeline"
---
...
In case, the default baselines functionality doesn't suit your project requirements, you can disable it using the [baselines](api/config/baselines.md) configuration option.
...
If you dynamically modify the display settings of the **gantt.config.baselines** config, you should use the [adjustTaskHeightForBaselines](api/method/adjusttaskheightforbaselines.md) method
...
2. [Creating a custom baseline element](guides/baselines.md) for adding into the timeline.
...
You can get the baselines of a particular task using the [getTaskBaselines](api/method/gettaskbaselines.md) method. 
...
To specify a text that should be displayed inside the baseline element, use the [baseline_text](api/template/baseline_text.md) template:
...
In case, the default deadlines functionality doesn't suit your project requirements, you can disable it using the [deadlines](api/config/deadlines.md) configuration option.

~~~js
gantt.config.deadlines = false;
~~~

After that you can customize the display of deadlines by [creating a custom deadline element](guides/baselines.md) for adding into the timeline.
...
Starting from v9.0, when [auto scheduling](guides/auto-scheduling.md) is enabled and works in the Constraint mode ([auto_scheduling_compatibility](api/config/auto_scheduling_compatibility.md) is set to *false*), Gantt will automatically display constraint dates in the chart.
...
The display of constraints can be controlled using the `show_constraints` option in the [auto_scheduling](api/config/auto_scheduling.md) config. 
