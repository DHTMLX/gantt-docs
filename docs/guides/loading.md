...
To load data from a server, use the [](api/method/load.md) method:
...
The *load* method will send an AJAX request to the specified url and will expect a response with data in [one of the supported formats](guides/supported-data-formats.md).
For example:
...
:::note
See detailed instructions and code samples for various platforms in the article [](guides/server-side.md#loadserverside).
:::
...
:::note
See all supported data formats in the article [Supported Data Formats](guides/supported-data-formats.md).
::: 


## Loading Task Dates {#loadingtaskdates}
...
**Related sample**: [Backward planning](https://docs.dhtmlx.com/gantt/samples/01_initialization/18_backward_planning.html)
...
**2a)** To change the format of the end dates of tasks in the gantt (i.e. to include the end date in the duration of the tasks), you can redefine the [task_end_date](api/template/task_end_date.md)
template.
...
But if you apply the [task_end_date](api/template/task_end_date.md) and [grid_date_format](api/template/grid_date_format.md)
...
If you are using the [format for inclusive end dates](api/template/task_end_date.md) of tasks and want to make it work correctly with [inline editing](guides/inline-editing.md) in the grid, you have to create a special editor for editing inclusive end dates of tasks, as in:
...
### Properties of a task object {#task_properties}

:::note
The full list of properties of a task object is given in the [Task properties](guides/task-properties.md) article.
:::

The default date format for JSON and XML data is **"%d-%m-%Y %H:%i"** (see the [date format specification](/guides/date-format/)).
...
Date formats that are not supported by the [date_format](api/config/date_format.md) config can be parsed manually via the [parse_date](api/template/parse_date.md) template.

### Properties of a link object {#link_properties}

:::note
The full list of properties of a link object is given in the [Link properties](guides/link-properties.md) article.
:::
...
See examples of data with custom properties [here](/guides/supported-data-formats#custom-properties-in-data).
...
<ul>
  ...
  <li><b>type</b> - (<i>string</i>) optional, the [type](guides/task-types.md) of the task. </li>
  <li><b>readonly</b> - (<i>boolean</i>) optional, can mark task as [readonly](guides/readonly-mode.md#readonlymodeforspecifictaskslinks). </li>
  <li><b>editable</b> - (<i>boolean</i>) optional, can mark task as [editable](guides/readonly-mode.md#readonlymodeforspecifictaskslinks). </li>
  </ul>
  ...
  <li><b>type</b> - (<i>string</i>) the type of the dependency:<ul><li>0 - 'finish_to_start'</li><li>1 - 'start_to_start'</li> <li>2 - 'finish_to_finish'</li><li>3 - 'start_to_finish'</li></ul> </li> 
  <li><b>lag</b> - (<i>number</i>) optional, [task lag](/guides/auto-scheduling#settinglagandleadtimesbetweentasks). </li>
  <li><b>readonly</b> - (<i>boolean</i>) optional, can mark link as [readonly](guides/readonly-mode.md). </li>
  <li><b>editable</b> - (<i>boolean</i>) optional, can mark link as [editable](guides/readonly-mode.md). </li>
  </ul>
</ul> 
