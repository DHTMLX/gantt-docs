...
2. [Read-only mode for specific tasks](guides/readonly-mode.md#readonlymodeforspecifictaskslinks)


## Read-only mode for the entire gantt {#readonlymodefortheentiregantt}

To make the entire Gantt chart read-only, set the [readonly](api/config/readonly.md) option to *true*.
...
~~~
By default, the mentioned behavior is binded to the 'editable' property of a task/link. You can change the target property using the [editable_property](api/config/editable_property.md) configuration option:
...
## Read-only mode for specific tasks/links {#readonlymodeforspecifictaskslinks}
...
In case you want to show the lightbox for read-only tasks, you can manually call it using [gantt.showLightbox(id)](api/method/showlightbox.md):
...
By default, the read-only behavior is binded to the 'readonly' property of a task/link. But you can change the target property using the [readonly_property](api/config/readonly_property.md) configuration option:
...
- manage their editability manually, e.g. by blocking the [onBeforeLightbox](api/event/onbeforelightbox.md) and [onBeforeTaskDrag](api/event/onbeforetaskdrag.md) events 
- dynamically update the 'editable_property' each time the task is loaded, added or updated ([onTaskLoading](api/event/ontaskloading.md), [onTaskCreated](api/event/ontaskcreated.md), [onAfterTaskUpdate](api/event/onaftertaskupdate.md)):
