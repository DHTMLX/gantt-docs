...
Coloring tasks allows you to highlight specific tasks to draw users' attention.

![coloring_tasks](/img/coloring_tasks.png)

To set a custom style for tasks, you can use one of the following approaches:

1. [To redefine the default tasks' template](guides/colouring-tasks.md#redefiningthetaskstemplate)
2. [To set style values in the properties of the task object](guides/colouring-tasks.md#specifyingstyleinthepropertiesofataskobject)
3. [To generate styles from data](guides/colouring-tasks.md#loadingcolorswithdata)

## Redefining the task's template {#redefiningthetaskstemplate}

To set a custom style for the task through a template, use the [task_class](api/template/task_class.md) template. For example, to color tasks depending on their priority, use the code as in:
...
:::note
To style other aspects of tasks, use the templates listed in the [Templates of the Timeline Area](guides/timeline-templates.md) article.
:::

A similar approach can be applied to links. Read more about it [here](guides/colouring-lines.md#redefiningthelinkstemplate).

## Specifying style in the properties of a task object {#specifyingstyleinthepropertiesofataskobject}
...
**Setting the task's color in the task object**
~~~js
const data = {
    tasks: [
...
If at least one of the properties of a task object is assigned, the task receives additional class **"gantt_task_inline_color"**.
...
A similar approach can be applied to links. Read more about it [here](guides/colouring-lines.md#specifyingcolorinthepropertiesofthelinkobject).

## Loading colors with data {#loadingcolorswithdata}
...
- Load options to the page, either by [using the gantt data format](guides/supported-data-formats.md#jsonwithcollections) or manually via a custom xhr
...
If you get resources from the [resource datastore](api/config/resource_store.md), you need to use `personOption.id` instead of `personOption.key` for the resource id.
