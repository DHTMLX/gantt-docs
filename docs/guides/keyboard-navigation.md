...
In order to use keyboard navigation in the Gantt chart, you need to enable the **keyboard_navigation** plugin using the [gantt.plugins](api/method/plugins.md) method.
...
The API of the **gantt.ext.keyboardNavigation** object is given in the article [Keyboard Navigation Extension](guides/keynav-ext.md).

There are two variants of keyboard navigation available:

- navigation by task rows

To enable it, set the [keyboard_navigation](api/config/keyboard_navigation.md) property to *true*.

- navigation by task cells  

To use this type of navigation, set the [keyboard_navigation_cells](api/config/keyboard_navigation_cells.md) property to *true*.
...
To return focus back to Gantt, you need to use the [focus](api/method/focus.md) method. When Gantt gets focus again, it places the focus on the active element inside, 
...
:::note
You can find an example of how to implement the ability to copy/paste tasks via the **Ctrl+C/Ctrl+V** combinations in the [related](guides/how-to.md#how-to-copy-and-paste-tasks) article.
:::
...
## Built-in Shortcut for Horizontal Timeline Scrolling {#builtinshortcutforhorizontaltimelinescrolling}
...
From version 6.3 you can either set the **Alt** or **Meta** key in the combination with mouse wheel instead of default **Shift** key via the [horizontal_scroll_key](api/config/horizontal_scroll_key.md) property:

~~~js
gantt.config.horizontal_scroll_key = "altKey";
~~~

or disable the horizontal scroll by setting the [horizontal_scroll_key](api/config/horizontal_scroll_key.md) property to *false*:
