---
title: "Tooltips for Gantt Elements"
...
shows a tooltip after the delay set by [tooltip_timeout](api/config/tooltip_timeout.md). If the config is not set, a small default delay is used.
...
hides the currently displayed tooltip after a delay set by [tooltip_hide_timeout](api/config/tooltip_hide_timeout.md). If the config is not set, a small default delay is used.

## Tooltips for different elements {#tooltipsfordifferentelements}
...
Note, the [gantt.ext.tooltips.tooltipFor()](#tooltipfor) method must be called after the Gantt initialization is complete. For instance, you can specify the method inside the [onGanttReady](api/event/onganttready.md) event handler like this:
...
**Related sample** [Gantt. Custom tooltips for cells](https://snippet.dhtmlx.com/6kb5gm39)

A tooltip added in this way will follow the mouse pointer and use the settings *[tooltip_offset_x](api/config/tooltip_offset_x.md)*, *[tooltip_offset_y](api/config/tooltip_offset_y.md)*, *[tooltip_timeout](api/config/tooltip_timeout.md)*,
