...
A complex control used to [assign multiple resources and their quantity to a task](guides/resource-management.md#assigningresources).
...
:::note
By default the resource control is mapped to the property specified in the [resource_property](api/config/resource_property.md) config, so the **map_to** option can be omitted.
:::
:::note
...
If you use the default resource Datastore created by Gantt, the [resource control](guides/resources.md) initialized without the **options** parameter, will be connected to the **gantt.serverList("resourceOptions")** collection. This collection will be populated with the resources from the resource datastore. You can access options by code:
...
Generally, to set values for the **resources** control, use the [options](api/config/lightbox.md) parameter:
...
Items in the [options](api/config/lightbox.md) parameter have 3 mandatory properties:
