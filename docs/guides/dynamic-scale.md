---
title: "Dynamic Change of Scale Settings"
 ...
 *For example, to change the scale's unit from "month" to "day", use the **unit** property of the [scales](api/config/scales.md) property*.
2. Redefine the related template (if required). 

 *For example, to [highlight weekends in the scale](guides/highlighting-time-slots.md), use the [scale_cell_class](api/template/scale_cell_class.md) template*.
2. Redraw the Gantt chart with the [render](api/method/render.md) method.
