...
:::note
Check the [Task end date display & Inclusive end dates](guides/loading.md#taskenddatedisplayampinclusiveenddates) article to read about the format of the end date of a task.
:::


## Enabling Work Time Calculation

To provide calculating tasks' duration in work time, use the [work_time](api/config/work_time.md) option:
...
Please note that the [skip_off_time](api/config/skip_off_time.md) configuration option is available only in the PRO version.
...
:::note
Depending on the value of [duration_unit](api/config/duration_unit.md), dhtmlxGantt calculates the tasks' duration in different time units (e.g. if
...
## Task duration in decimal format {#taskdurationindecimalformat}
...
"0.5 hours", "3.75 hours") via the [Duration Formatter](guides/formatters-ext.md) module.
...
:::note
The duration of tasks can be represented as a fraction of an hour, day or any other supported by the [duration_unit](api/config/duration_unit.md) config unit, except for minutes.
:::
...
- set [duration_unit](api/config/duration_unit.md) to minute
...
Pay attention that you need to store task durations in a smaller unit than the units of the values displayed in decimal format. To put it simply:
    ...
    - if you want a user to be able to specify durations as a fraction of an hour (e.g. "0.5 hours"), you need to set [duration_unit](api/config/duration_unit.md) to minute 


    - If you want a user to be able to specify durations as a fraction of a day, you need to set [duration_unit](api/config/duration_unit.md) to hour. In this case, users will be able to enter the duration of the task as "0.5 day", but "0.5 hour" will be rounded up to 1 hour, since the duration will be stored in integer hours.
...
To enable this drag and drop, you need to disable [round_dnd_dates](api/config/round_dnd_dates.md) and set an appropriate value to [time_step](api/config/time_step.md).
:::
For example:
...
:::note
If you already have Gantt with the duration of tasks stored in minutes, hours or any other unit, you can also use the [Duration Formatter](guides/formatters-ext.md) module to present the durations in decimal format.
:::
...
To change the default working time, use the [setWorkTime](api/method/setworktime.md) method:
...
There is the ability to configure different working time rules for different periods of time by using the **customWeeks** attribute of the [setWorkTime](api/method/setworktime.md) method. For instance, you can change the default working time for winter months:
...
To specify the working time not only from an hour to hour (e.g."8:00-12:00") but also including minutes (e.g. "8:15-12:45"), set the [duration_unit](api/config/duration_unit.md) config to *"minute"*.
...
:::note
The format of the working time that was used up to version 7.0
...
Note, each next call of the method for the same date will re-write the previous working-time rule. So, if you need to unset some rule, call the [setWorkTime](api/method/setworktime.md) method with other configuration: 
...
You can unset a working time by using the [unsetWorkTime](api/method/unsetworktime.md) method:
...
To check whether the specified date is working time, use the [isWorkTime](api/method/isworktime.md) method:
...
To get the working hours of the specified date, use the [getWorkHours](api/method/getworkhours.md) method:

~~~js
gantt.getWorkHours(new Date(2025, 3, 30)); // -> ["8:00-17:00"]
~~~

To get the closest working day to the specified date, use the [getClosestWorkTime](api/method/getclosestworktime.md) method:
...
### Repeating the specific working time
...
The current version of dhtmlxGantt doesn't provide any configs for setting such type of the working time. 

The library allows you only: 
...
To color the day-off times in the chart area, use the [timeline_cell_class](api/template/timeline_cell_class.md) template:
...
Learn more in the [Highlighting Time Slots](guides/highlighting-time-slots.md) article.

:::note
To hide the day-off time, use the technique described in the article - [Hiding Time Units in the Scale](guides/custom-scale.md).
:::


## Multiple Work Time Calendars {#multipleworktimecalendars}
...
A new calendar instance can be created using the [createCalendar](api/method/createcalendar.md) method.
...
to the [createCalendar](api/method/createcalendar.md) method
...
After you've created a calendar, you have to add it into Gantt with the help of the [addCalendar](api/method/addcalendar.md) method. Once again, there are two possibilities:
...
**Related sample**: [Different worktimes for different time periods](https://docs.dhtmlx.com/gantt/samples/09_worktime/12_calendar_ranges.html)


### Changing working hours

You can change the working time for separate days of the necessary calendar via the [setWorkTime()](api/method/setworktime.md) method:
...
To get the object of the global Gantt calendar using the [getCalendar](api/method/getcalendar.md) method:

~~~js
const calendar = gantt.getCalendar(id);
~~~

The *calendar* object is an instance of the [calendar](api/other/calendar.md) interface.
...
This calendar is used by the [work time methods](guides/working-time.md#global-settings), when there is no other calendar specified. It is assigned to tasks by default.


#### Getting the current calendar of a task

To get the object of a working calendar assigned to a particular task, apply the [getTaskCalendar](api/method/gettaskcalendar.md) method. You need to pass the task object to the method:
...
**Related sample**: [Task level calendars](https://docs.dhtmlx.com/gantt/samples/09_worktime/06_task_calendars.html)
...
The [work time methods](guides/working-time.md#global-settings) of the Gantt object can be used to calculate the time duration of a specific task, without accessing its calendar manually. 
...
To get all the calendars added into Gantt (both the global one and those assigned to separate tasks), make use of the [getCalendars](api/method/getcalendars.md) method:

~~~js
const calendars = gantt.getCalendars();
~~~

The method returns an array of [Calendar interface](api/other/calendar.md) objects.


### Deleting calendars

In case, you don't need a calendar anymore, you can easily remove it via the [deleteCalendar](api/method/deletecalendar.md) method.
...
## Assigning Calendar to Task {#assigningcalendartotask}
...
You can modify the name of the task property responsible for binding a calendar to a task via the [calendar_property](api/config/calendar_property.md) configuration option:
...
## Assigning Calendar to Resource {#assigningcalendartoresource}
...
- define the property of a task object that will store a resource id via the [resource_property](api/config/resource_property.md) configuration attribute. In the example below the property named **user** will store ids of users:

~~~js
gantt.config.resource_property = "user";
~~~

- use the [resource_calendars](api/config/resource_calendars.md) configuration option to add the desired calendar for each user and group calendars into one object.
...
id of the calendars returned by the [addCalendar](api/method/addcalendar.md) method.
...
### Merging multiple calendars {#mergingcalendars}
...
Setting the [dynamic_resource_calendars](api/config/dynamic_resource_calendars.md) config to *true* will enable this feature automatically: 
...
**Related sample**: [Merge work Calendars of different resources](https://docs.dhtmlx.com/gantt/samples/09_worktime/10_merge_calendars.html)


But you can also merge calendars manually with the help of the [mergeCalendars](api/method/mergecalendars.md) method:
...
Learn the logic of how merging of working hours is performed in the [mergeCalendars()](api/method/mergecalendars.md) article.
...
To enable this functionality, you need to set the [inherit_calendar](api/config/inherit_calendar.md) configuration option to *true*. By default this option is disabled.
...
**Related sample**: [Project level calendars](https://docs.dhtmlx.com/gantt/samples/09_worktime/08_project_calendars.html)
...
**Related sample**: [Toggle working time settings and move the task to the working date](https://snippet.dhtmlx.com/6cvo9dy9)

**Related sample**: [Toggle working time settings and recalculate the task's end dates](https://snippet.dhtmlx.com/wb8vc82p)
...
Check the detailed description of **scale projection** possibilities [in the related guide](guides/configuring-time-scale.md#workhourawaretaskbarsrenderingindayweekscales).
