...
Клонируйте или загрузите репозиторий [DHTMLX/skills](https://github.com/DHTMLX/skills) и скопируйте соответствующую папку навыков (`dhtmlx-js-gantt`, `dhtmlx-react-gantt`, или `dhtmlx-angular-gantt`) в каталог навыков вашего проекта (например, `.claude/skills/` для Claude Code, `.cursor/skills/` для Cursor).
