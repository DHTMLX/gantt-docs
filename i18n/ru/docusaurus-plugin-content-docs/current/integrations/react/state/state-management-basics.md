...
В этом шаблоне вы держите все основные коллекции в состоянии и передаёте их как пропсы (`tasks`, `links`, `resources`, `resourceAssignments`). Всякий раз, когда пользователь изменяет задачи или ссылки внутри Gantt (например, создаёт или удаляет задачу), Gantt вызывает обработчик. В этом обработчике вы обновляете своё состояние React новыми или удалёнными данными. Как только состояние обновлено, React повторно отрисовывает компонент **ReactGantt**, который, в свою очередь, читает обновлённые пропсы из последнего состояния.

При типизации состояния используйте `SerializedTask` для задач и `SerializedLink` для ссылок. Эти типы представляют формату данных, с которым работает пользователь — поля даты принимают `Date | string`, и отсутствуют внутренние свойства с префиксом `$`. Используйте `Task` и `Link` только при работе с данными внутри обработчиков событий Gantt, где данные уже распарсены Gantt.
