...
:::note
Переход с более ранней версии? Ознакомьтесь с [руководством по миграции](migration.md) для необходимых изменений и шагов обновления.
:::

## 10.0

<span class='release_date'>11 июня 2026 г. Крупный выпуск</span>

Это обновление вносит изменения в структуру пакета Gantt и поведение функциональности. Обязательно проверьте [замечания по миграции](migration.md#91---100), чтобы избежать возможных проблем.

### Новая функциональность

- DHTMLX Gantt Community Edition официально выпущен — бесплатное издание теперь распространяется под [лицензией MIT](migration.md#gpl-to-mit)
- [Angular Gantt](integrations/angular.md) обёртка официально выпущена
- [Vue Gantt](integrations/vue.md) обёртка официально выпущена
- Zoom-to-fit доступен «из коробки»: [`gantt.ext.zoom.zoomToFit()`](guides/zoom.md#methods) выбирает наиболее детальный уровень зумирования, который позволяет поместить все задачи в таймлайн без горизонтальной прокрутки, а [`resetZoom()`](guides/zoom.md#methods) возвращает к предыдущему масштабу
- [React Gantt](integrations/react.md) теперь предоставляет готовые React-хуки для наиболее часто используемых API Gantt и поставляется с рабочими значениями по умолчанию для гистограммы ресурсов, устраняя большую часть boilerplate, ранее необходимого для доступа к нативному экземпляру
- Добавлена поддержка дополнительных [локалей](guides/localization.md) — включая упрощённый китайский, традиционный китайский, кантонский, тайский и вьетнамский — и существующие переводы улучшены

### Обновления

- Движок автоматического планирования глубоко переработан, что позволило устранить давние проблемы
- Обновлены типы TypeScript, смотрите обзор [Модели данных](guides/data-model.md) для деталей
- Парсинг и форматирование дат теперь используют единую CSP-безопасную реализацию
- [Zoom extension](guides/zoom.md) теперь поставляется с готовыми именованными [значениями уровней по умолчанию](guides/zooming.md#default-zoom-levels), поэтому можно инициализировать его без пользовательской конфигурации `levels` (`gantt.ext.zoom.init()`)
- Интервал-старт помощников [`gantt.date`](api/other/date.md) теперь **чистый** — они возвращают новый `Date`, а не изменяют переданный объект
- [React Gantt](integrations/react.md) теперь передаёт **экземпляр Gantt** в компонент [customLightbox](integrations/react/overview.md#by-providing-a-custom-component-via-the-customlightbox-prop), что позволяет прямой доступ к API Gantt из настраиваемого редактора
- Обёртки [React Gantt](integrations/react.md), [Vue Gantt](integrations/vue.md) и [Angular Gantt](integrations/angular.md) теперь по умолчанию HTML-экранируют строковые значения, возвращаемые функциями шаблонов, чтобы предотвратить XSS-атаки. Это касается функций `templates`, `config.columns[].template` и `config.scales[].format`.

### Исправления

Переработанный движок Auto Scheduling исправил ряд давно существовавших ошибок планирования, ограничений и критического пути:

- Исправлена проблема, когда задачи с поведением ASAP перемещались до даты `project_start`, если их соседние задачи имели ограничения `snet`/`snlt`
- Исправлено неожиданное смещение соседних задач к более ранним датам во время [Auto Scheduling], когда был включён параметр `auto_scheduling_move_projects`
- Исправлена проблема, когда дочерние задачи перераспределялись на неверные даты, если их родительский проект использовал другой [календарь], чем его дети
- Исправлена проблема, когда дочерние задачи перемещались на более позднюю дату, если их связанный предшественник имел нерабочее время, а дочерняя задача использовала полный календарь
- Исправлена проблема, когда дочерние задачи с собственными календарями планировались на будущую дату, если включён параметр `inherit_calendar` для многоуровневых проектов
- Исправлена ошибка, когда задачи могли перераспределиться после окончания проекта (`project_end`) во время обратного [Auto Scheduling]
- Исправлена ошибка, когда [Auto Scheduling] не изменял размер задач после изменения настроек времени работы `work time`, даже если задача не требовала перемещения
- Исправлена ошибка, возникавшая, если у задачи был констрейнт, но свойство `auto_scheduling` было установлено в `false`
- Исправлена проблема, при которой событие [onBeforeTaskAutoSchedule](api/event/onbeforetaskautoschedule.md) не срабатывало для задач без связей, поэтому они не могли быть исключены из расписания
- Исправлен некорректный расчёт Total Slack и [критического пути](guides/critical-path.md) для связей Start-to-Finish и Start-to-Start
- Исправлена проблема, когда предшественник не выделялся как критический, когда отрицательная задержка делала дату окончания наследника раньше конца предшественника
- Исправлена ошибка `invalid end_date argument` в [getTotalSlack](api/method/gettotalslack.md) при наличии в проекте лишь одной родительской задачи без подзадач
- Исправлена проблема, когда подзадача с поведением ASAP прыгала назад-вперёд между датами на каждом запуске Auto Scheduling, если у сестринской задачи был констрейнт MSO/MFO, и родительский проект был связан с помощью Finish-to-Finish или Start-to-Finish
- Исправлено применение логики ограничения к MSO/MFO задачам, когда был включён параметр `auto_scheduling_compatibility`
- Исправлено некорректное планирование связанной подзадачи, если её связь использовала отрицательную задержку, большую чем длительность наследника, и родительский проект был соединён ссылкой Finish-to-Finish или Start-to-Finish

Другие исправления:

- Исправлена проблема, когда вехи не конвертировались в проекты при использовании параметра auto_types, когда к ним добавлялась подзадача
- Исправлена проблема в [React Gantt](integrations/react.md), когда ISO-строки дат неправильно обрабатывались без явных шаблонов `parse_date`/`format_date`
- Исправлена проблема в [React Gantt](integrations/react.md), когда задача могла потерять родителя после загрузки нового набора данных, если дочерний элемент располагался выше родителя в данных
- Исправлена проблема в [React Gantt](integrations/react.md), когда вертикальный маркер повторного упорядочивания не растягивался на всю ширину грида

## 9.1.4

<span class='release_date'>28 апреля 2026 г. Исправления ошибок</span>

### Новая функциональность

- Бета-версия обёртки Angular Gantt

### Исправления

- Исправлена проблема, когда задачи-роллап не могли перетаскиваться на сенсорных устройствах
- Исправлена проблема, когда Auto Scheduling перемещал связанный проект на будущую дату, если в проекте были несвоевременнs задачи
- Исправлена проблема, когда маркер перетаскивания повторного упорядочивания и заполнитель задачи имели некорректные размеры, когда сетка была прокручиваемой
- Исправлена проблема, когда DataProcessor не использовал формат даты ISO по умолчанию, если задачи загружались с датами в формате Date
- Исправлена ошибка скрипта, возникавшая, когда [drag_links](api/config/drag_links.md) был отключён и строка разделённой задачи была выше родительской строки
- Исправлено пропадание шкал на временной шкале после изменения уровня зума

## 9.1.3

<span class='release_date'>16 марта 2026 г. Выпуск исправлений</span>

### Исправления

- Исправлена проблема, при которой функциональность [zoom](guides/zooming.md) не работала, когда главная грид и таймлайн изначально не отображались в [layout](guides/layout-config.md)
- Исправлена проблема, при которой ячейки шкалы пропадали в [таймлайне ресурсов](guides/resource-management.md#resourceviewpanel) после изменения уровня масштабирования
- Исправлена проблема, при которой ячейки таймлайна исчезали после прокрутки таймлайна вправо и изменения [zoom level](guides/zooming.md) во время экспорта
- Исправлена проблема, при которой ячейки шкалы исчезали при изменении размера окна, когда таймлайн был прокручен вправо и включено расширение [Zoom](guides/zooming.md)
- Исправлена проблема, когда задачи исчезали после изменения [zoom level](guides/zooming.md) при включенном [open_split_tasks](api/config/open_split_tasks.md)
- Исправлена проблема, при которой задачи сворачивания ([rollup](guides/milestones.md#rolluptasksandmilestones)) некорректно отображались на больших наборах данных до взаимодействия с родительской задачей
- Исправлена проблема, когда [несогласованные задачи](guides/unscheduled-tasks.md) не отображались, если [show_unscheduled](api/config/show_unscheduled.md) был установлен в `false` и был сконфигурирован диапазон дат
- Исправлена проблема, когда [resource grid](guides/resource-management.md#resourceviewpanel) исчезал при включенной навигации по клавиатуре и инициализированном datastore ресурса без дерева
- Исправлены стили переполнения всплывающего окна [Quick Info](guides/quick-info.md) для длинного содержимого

### Обновления

- Добавлена автоматическая детекция и поддержка строк в формате даты ISO в данных задач. Даты ISO 8601 (например, `"2026-01-06"`, `"2026-01-06T10:30:45Z"`) теперь парсятся и сериализуются автоматически без необходимости ручных шаблонов

## 9.1.2

<span class='release_date'>27 февраля 2026 г. Выпуск исправлений</span>

- Исправлена проблема, когда Gantt мог исчезать после повторного упорядочивания задач или создания связей, когда режим `touch` был включен и Gantt рендерился внутри Shadow DOM
- Исправлена настройка параметра `initial_scroll` в [React Gantt](integrations/react.md), чтобы диаграмма прокручивалась к первой задаче при инициализации
- Исправлена проблема, когда назначения задач не обновлялись корректно после перетаскивания, если изменения происходили внутри [gantt.batchUpdate()](api/method/batchupdate.md)
- Исправлена утечка памяти в рендерере фоновой строки задачи
- Исправлена проекция шкал в [React Gantt](integrations/react.md)
- Исправлено отсутствие отрисовки [базовых линий](guides/inbuilt-baselines.md), добавленных через datastore
- Исправлена проблема, когда Gantt переставал работать после предоставления неверной конфигурации [layout](guides/layout-config.md)

## 9.1.1

<span class='release_date'>15 декабря 2025 г. Выпуск исправлений</span>

### Исправления

- Исправлена ошибка в [React Gantt](integrations/react.md), когда пользователь передал свойство `links` без `tasks`
- Исправлена проблема в [React Gantt](integrations/react.md), где повторно загруженные задачи не имели идентификаторов связей в своих свойствах `$source` и `$target`
- Исправлена проблема, когда Gantt открывал встроенный редактор после нажатия на клавишу **Meta**
- Исправлена корректность работы формата даты `%w` ([date format](guides/date-format.md)) в CSP-производственных средах
- Исправлена проблема, которая препятствовала корректному отображению [групп задач](api/method/groupby.md) как [разделённых задач](guides/split-tasks.md) при наличии параметра `render="split"`
- Исправлена проблема с исчезновением ресурса нажатия клавишей **Tab** после редактирования ячейки ресурса

### Обновления

- Теперь можно настраивать высоту строки и панели задач для [виртуальных задач](api/method/groupby.md)

## 9.1

<span class='release_date'>5 ноября 2025 г. Небольшое обновление</span>

### Разрушительные изменения

Это обновление вносит некоторые изменения в структуру пакета Gantt и поведение функциональности. Убедитесь, что вы ознакомились с 
[Migration notes](migration.md#90---91), чтобы обезопасить себя.

### Новая функциональность

- Добавлен модуль [Multi-user backend](guides/multiuser-live-updates.md) для обновлений в реальном времени
- Добавлен [Remote updates API](guides/multiuser-live-updates.md#remote-updates-api) для управления синхронизацией данных в реальном времени
- Введён новый [Resource Assignments control](guides/resource-assignments.md), расширяющий функциональность контроля [Resources](guides/resources.md)
- Возможность [отображать задачи в Day/Week масштабе](guides/configuring-time-scale.md#workhourawaretaskbarsrenderingindayweekscales) на основе фактических рабочих часов, а не фиксированного 24-часового периода
- Возможность [рендерить разделённые подпроекты] в зависимости от состояния родной строки (`render="split"`)

### Обновления

- Добавлена возможность фиксировать размер ячеек [Timeline cells](guides/configuring-time-scale.md#fixed-column-width)
- Добавлена поддержка указания [дат в конфигурации календаря](api/method/addcalendar.md)
- Добавлен [конфигурационный объект](api/config/auto_scheduling.md) для [Auto Scheduling](guides/auto-scheduling.md) для упрощения настройки
- Улучшена поддержка сенсорных экранов на устройствах Windows
- Удалён устаревший API **subscales**

### Исправления

- Исправлена проблема, когда [](api/method/exporttoexcel.md) с `visual:true` и пользовательские данные приводили к некорректному отображению, если временная шкала была скрыта
- Исправлена проблема, когда [](api/method/exporttopdf.md) с `additional_settings.slice_archive` приводила к неполному отображению Gantt
- Исключены несогласованные задачи из экспорта Excel
- Добавлена поддержка экспорта [split tasks](guides/split-tasks.md) в Excel
- Исправлено некорректное отображение [Resource Histogram](guides/resource-management.md#resourceviewpanel) после изменения размера окна
- Исправлена проблема перекрытия задач при смене уровня масштабирования

## 9.0.15

<span class='release_date'>19 сентября 2025 г. Выпуск исправлений</span>

### Исправления

- Исправлена проблема, когда Gantt игнорировал конфигурацию рабочих дней `customWeeks` при вызове метода [setWorkTime](api/method/setworktime.md)
- Улучшена поддержка [Shadow DOM](https://html.spec.whatwg.org/multipage/custom-elements.html) внутри элементов Gantt
- Исправлена проблема в [React Gantt](integrations/react.md), когда несколько экземпляров компонента приводили к дублированию шаблонов

## 9.0.14

<span class='release_date'>31 июля 2025 г. Выпуск исправлений</span>

### Исправления

- Исправлена ошибка скрипта после закрытия окна [Modalbox](guides/message-boxes.md)
- Предотвращено создание пустой строки в дереве после повторного разбора данных в режиме [grouped](api/method/groupby.md)
- Убедитесь, что метод [setWorkTime](api/method/setworktime.md) корректно применяет **customWeeks** в Firefox
- Исправлена проблема, из-за которой вызов [setTasks](integrations/react.md) с пустым набором данных ломал компонент React Gantt

## 9.0.13

<span class='release_date'>26 июня 2025 г. Выпуск исправлений</span>

### Исправления

- Исправлена проблема, из-за которой Gantt становился непригодным после создания и удаления дубликата связи
- Убедитесь, что ссылки-плейсхолдеры отображаются корректно на тач-устройствах
- Исправлена проблема, когда `end_date` проекта не обновлялся, если у него был один дочерний этап
- Предотвращено ненужное повторное прорисование элементов ресурса при использовании настраиваемого datastore
- Исправлена неправильная работа вызова [gantt.addLink](api/method/addlink.md) с неявным состоянием внутреннего datastore
- Предотвращено добавление свойства `!nativeeditor_status` к [custom router](guides/server-side.md#customrouting) в DataProcessor
- Исправлено некорректное обращение с изменениями пропсов `tasks` и `links` в компоненте [React Gantt](integrations/react.md)

## 9.0.12

<span class='release_date'>19 июня 2025 г. Выпуск исправлений</span>

### Исправления

- Обеспечено прохождение skin contrast-white тестов доступности цвета
- Исправлена проблема, когда [dynamic loading](guides/dynamic-loading.md) переставал работать после вызова [gantt.clearAll()](api/method/clearall.md)
- Предотвращено рендеринг [Tooltip](guides/tooltips.md) вне контейнера Gantt при отображении длинного текста
- Исправлена проблема, когда серверные обновления [server updates](guides/server-side.md) не завершались при активной навигации клавиатурой [keyboard navigation](guides/keyboard-navigation.md)
- Исправлено некорректное поведение пользовательских имён классов, применённых к [milestone baselines](guides/inbuilt-baselines.md)

### Обновления

- [React Gantt](integrations/react.md) теперь совместим с фреймворками **Next.js** и **Remix** без дополнительной конфигурации
