...
- `calendars` - (required) *Calendar[] | Calendar* - массив объектов Calendar или первый объект календаря
-  `calendar2` -    (optional) *Calendar*   - необязательный, второй объект календаря
...
You can also specify a set of [objects of calendars](api/other/calendar.md) as parameters of the **mergeCalendars** method:
...
:::note
Логика не учитывает [customWeeks](api/method/addcalendar.md). 
:::
