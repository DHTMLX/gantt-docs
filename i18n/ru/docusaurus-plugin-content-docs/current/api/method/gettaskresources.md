...
description: "возвращает массив уникальных ресурсов, назначенных конкретной задаче из хранилища данных"
---
...
:::note
 Метод **getTaskResources** недоступен, если [process_resource_assignments](api/config/process_resource_assignments.md) отключён.
:::

Метод возвращает массив объектов **resourceItem**, которые имеют следующие свойства:

- **id** - (*string | number*) - идентификатор ресурса
- **open?** - (*boolean*) - указывает, развёрнут ли элемент ресурса в дереве (*true*) или свернут (*false*)
- **parent?** - (*string | number*) - идентификатор родительского элемента ресурса
- **text?** - (*string*) - имя ресурса
- **unit?** - (*string*) - единица для назначений
