...
Вы можете использовать этот метод для перерисовки задачи после изменения её свойств. В отличие от [updateTask](api/method/updatetask.md), этот метод не вызывает [DataProcessor](guides/server-side.md), и обновления не будут отправлены на сервер.
