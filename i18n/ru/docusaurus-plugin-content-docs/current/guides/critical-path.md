...
## Логика критического пути {#critical-path-logic}
...
## Настройка поведения критического пути {#customizing-the-critical-path-behaviour}
...
## Критический путь и запас в v10.0 {#v2-analysis}

В версии v10.0 переработан расчет запаса и критического пути. Результаты больше не зависят от опций автоспланирования, таких как [move_projects](api/config/auto_scheduling.md#move_projects) и [gap_behavior](api/config/auto_scheduling.md#gap_behavior) — одни и те же данные всегда дают одинаковые значения запаса и критического пути.

Несколько связанных заметок о поведении:

- Задача считается критической, когда ее общий запас не больше нуля. Завершённые задачи (когда включена [use_progress](api/config/auto_scheduling_use_progress.md)) и задачи, принадлежащие к зависимой петле, никогда не являются критическими.
- `getTotalSlack` и `getFreeSlack` возвращают `0` (а не `undefined`) для задач, исключённых из расчёта — задач в петле зависимостей и завершённых задач, когда включен `use_progress`.

### Возврат к предыдущему вычислению

Предыдущее вычисление по-прежнему доступно через переходной флаг opt-out. Он будет удалён в v10.1, поэтому используйте его только во время перехода:
...
Смотрите [миграцию](migration.md#auto-scheduling-v2) для полного списка изменений в поведении.