...
## Отключение авто-распределения для конкретных задач {#disabling-auto-scheduling-for-specific-tasks}
...
- в v10.0 движок auto-scheduling был переработан. Публичный API и видимое поведение остались без изменений, за исключением случаев, которые ранее работали некорректно. См. [руководство по миграции](migration.md#auto-scheduling-v2) для полного списка изменений поведения.