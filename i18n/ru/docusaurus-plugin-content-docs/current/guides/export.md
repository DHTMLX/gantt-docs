...
:::note
Сервис бесплатный, но итоговый PDF/PNG-файл будет содержать водяной знак библиотеки.
Чтобы экспортировать без водяного знака, нужна действующая лицензия — результат экспорта будет доступен без водяного знака
...
<table class="webixdoc_links">
  ...
  <td>(<i>string</i>) задаёт конечную точку API для запроса. Может использоваться с локальной установкой сервиса экспорта. Значение по умолчанию — `https://export.dhtmlx.com/gantt`</td>
  </tr>
  <tr>
