...
## Липкие метки {#sticky-labels}

Начиная с v9.0, метки временной шкалы по умолчанию являются липкими. Это означает, что когда ширина ячейки значительно больше ширины её метки, метка будет оставаться видимой при прокрутке временной шкалы, оставаясь прикреплённой к области просмотра, пока естественным образом не прокрутится за её пределы. Это улучшает видимость меток шкалы, особенно при увеличении или уменьшении масштаба.

Чтобы вернуться к старому поведению, при котором метки центрируются внутри своих ячеек и не остаются видимыми при прокрутке, можно отключить липкие метки, установив свойство `sticky` объекта шкалы в `false`:
...
Вы также можете принудительно сделать метки липкими для определённой шкалы независимо от ширины ячейки, установив `sticky: true`. Это обеспечит постоянную липкость меток, даже если ширина метки меньше ширины ячейки:
