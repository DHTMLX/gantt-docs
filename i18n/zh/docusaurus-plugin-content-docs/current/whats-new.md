...
- 修复当 [column](guides/specifying-columns.md#wbscode) `name` 包含空格时，在 `onrender` 函数中抛出错误的问题
- 修复启用 [S-Curve Overlay](guides/baselines.md#extra-overlay-for-the-chart) 时，点击拖动 [Timeline](guides/extensions-list.md#advanced-drag-n-drop) 后 Gantt 仍保持只读模式的问题
- 修复使用 [Resource calendar](guides/resource-management.md) 且 [Resources](guides/resource-management.md#resourceviewpanel) 区域在 [Lightbox](guides/default-edit-form.md) 的 duration 区域下方时，任务持续时间被重置为 `0` 的问题
- 修复在 **React StrictMode** 下，Gantt 显示 "This is a Trial version" 警告时引发的控制台错误
- 修复当 [gantt.config.baselines.render_mode](api/config/baselines.md#details) 被禁用时，[adjustTaskHeightForBaselines](api/method/adjusttaskheightforbaselines.md) 函数未重新计算行高的问题
...
- 修复当子任务未调度且缺少日期参数时无法拖动 [project tasks](guides/task-types.md#project-tasks) 的问题
...
- 修复当 [onBeforeTaskChanged](api/event/onbeforetaskchanged.md) 事件返回 `false` 时，[project tasks](guides/task-types.md#project-tasks) 未重新绘制的问题
- 确保任务进度条在需要时能覆盖任务的全部宽度
- 移除只读任务链接中的无效 WAI-ARIA 属性，以提升 [无障碍性](guides/accessibility.md#wai-aria-attributes)
...
- 修复 [click_drag](guides/extensions-list.md#advanced-drag-n-drop) 扩展在触摸设备上无法使用的问题
...
- 修正包含 [placeholder task](api/config/placeholder_task.md) 的已排序任务的 [WBS code](guides/specifying-columns.md#wbscode) 计算
- 解决在折叠任务或 [调整 grid 大小](guides/layout-config.md#default-layout) 后，资源 timeline 的刻度单元消失的问题
- 确保在 [`gantt.silent`](api/method/silent.md) 函数内部同时添加父子任务时任务顺序正确
- 在编辑 [resource cells](api/template/resource_cell_value.md) 后保持焦点
...
- 确保 [predecessor inline editor](guides/inline-editing.md#linkformatter) 在链接 id 类型为数字时正常工作
...
- 确保在 [gantt.batchUpdate](api/method/batchupdate.md) 内添加任务时，[resource assignments](guides/resource-management.md#managingresourceassignments) 保持不变
...
- 解决影响 [resource panel](guides/resource-management.md#resourceviewpanel) 的 `fetchTasks` 和 [deepcopy_on_parse](api/config/deepcopy_on_parse.md) 配置相关问题
...
- 修复在 [Resource Grid](guides/resource-management.md#resourceviewpanel) 调整行高时报错的问题
- 确保 [Time control](guides/time.md#shuxing) 的 **year_range** 属性指定的区间包含最后一年
- 修正受 `line-height` CSS 样式影响的链接箭头位置
- 修复加载 [collections](guides/supported-data-formats.md#jsonwithcollections) 且无 `links` 数组时报错的问题
- 解决 `gantt.config.baselines` 为 falsy 但未显式设置为 `false` 时的问题
- 修复调用 [resetLayout](api/method/resetlayout.md) 后 [click_drag](guides/extensions-list.md#advanced-drag-n-drop) 不工作的问题
...
- 修复启用 `fetchTasks` 时，筛选后 [resource panel](guides/resource-management.md#resourceviewpanel) 显示异常的问题

## 9.0.4

<span class='release_date'>2024年12月3日。Bug 修复版本</span>

- 修复无法通过 [inline editor](guides/inline-editors-ext.md) 更改 [SNET constraint](guides/auto-scheduling.md#timeconstraintsfortasks) 日期的问题
- 修复在 [dynamic_resource_calendars](api/config/dynamic_resource_calendars.md) 配置被禁用时，具有单一 [resource assignment](guides/resource-management.md#managingresourceassignments) 的任务返回全局 [calendar](guides/working-time.md) 而非资源日历的问题
- 修复在 [reinitializing](api/method/init.md) Gantt 或 [resetting the layout](api/method/resetlayout.md) 后，通过 inline editor 更改开始日期时的 [constraint](guides/auto-scheduling.md#timeconstraintsfortasks) 日期问题
...
- 确保拖动任务后 Gantt 能保留任务的 [constraint](guides/auto-scheduling.md#timeconstraintsfortasks)
...
- 防止通过 lightbox 更新时 [resource assignment](guides/resource-management.md#managingresourceassignments) 被移除
...
- 确保 [resource panel](guides/resource-management.md#resourceviewpanel) 与 [zoom levels](guides/zooming.md#neizhisuofangmokuai) 保持同步
...
- 修复了调用 [resetLayout](api/method/resetlayout.md) 后，[S-Curve Overlay](guides/baselines.md#extra-overlay-for-the-chart) 插件功能失效的问题
- 防止 [predecessor editor](guides/inline-editing.md#types-of-editors) 创建重复链接
...
- 新增[暗色皮肤](guides/skins.md#darkskin)
...
- 修正了当子任务链接延迟为0时，[Auto Scheduling](guides/auto-scheduling.md#settinglagandleadtimesbetweentasks) 项目的调度问题
- 修复了具有不同行高的 [split tasks](guides/split-tasks.md) 的链接位置错误
- 确保 Gantt 能正确自动调度具有两级任务的项目
- 修复了当任务超出指定日期范围时，[resource_cell_value](api/template/resource_cell_value.md) 未返回固定日期"assignments"的问题
...
- 修复了当右侧有[列宽调整器](guides/specifying-columns.md#resizing)时，Grid 与 Timeline 联动滚动的问题
...
- 解决了当页面上有 [Resource Histogram](guides/resource-management.md#resourceviewpanel) 时，[键盘导航](guides/keyboard-navigation.md) 的问题
...
- 修复了当任务开始于最小日期之前但结束在显示日期范围内时，[Resource Histogram](guides/resource-management.md#resourceviewpanel) 的模板未被调用的问题
...
- 修复了 [DataProcessor](guides/server-side.md#resources_crud) 忽略 [gantt.createDataProcessor](api/method/createdataprocessor.md) 方法中自定义 header 的问题
...
- 修复了在禁用智能渲染时，添加 [Rollup](guides/milestones.md#rolluptasksandmilestones) 任务导致甘特图崩溃的问题
...
- 对 [WAI-ARIA 属性](guides/accessibility.md#wai-aria-attributes) 的使用进行了增强和修正，以提升无障碍性
...
- [导出 API 客户端](guides/extensions-list.md#export-service) 现已集成到 Gantt 的 GPL 版本中，以前仅包含在 PRO 版本中
- 在 [Node.js 版本的 Gantt](guides/using-gantt-on-server.md) 中增加对 https 导出 [服务器端点](guides/export.md#parameters-of-the-export-methods) 的支持
...
- 修复了 [DataProcessor](guides/server-side.md#resources_crud) 未跟踪 [资源数据存储](guides/resource-management.md#shiyongziyuanshitumianban) 变更的问题
...
- 修复了带有 [负时滞](guides/auto-scheduling.md#settinglagandleadtimesbetweentasks) 任务的 [自由时差](guides/critical-path.md#gettingfreeandtotalslack) 计算不正确的问题
...
- 修复了使用 [LinkFormatters](guides/formatters-ext.md#linkformatter) 时 [导出](guides/export-common.md) 出现的错误
- 修复了 [撤销扩展](guides/undo-redo.md) 与 [资源及资源分配](guides/resource-management.md) 配合使用时工作不正确的问题
- 更新了类型定义
- 提升了 [Rollup](guides/milestones.md#rolluptasksandmilestones) 任务渲染的性能
...
- 修复了当 [工作时间设置](guides/working-time.md#global-settings) 包含分钟部分时，[自动调度](guides/auto-scheduling.md) 与 [MSO、FNET 和 FNLT 约束](guides/auto-scheduling.md#timeconstraintsfortasks) 配合使用不正确的问题
...
- 资源管理更新:
    ...
    - 可通过 [DataProcessor](guides/server-side.md#resources_crud) 捕获资源和资源分配的变更
...
- Rollup 项和拆分任务的改进:
    - 可为单独的 [rollup 项](guides/milestones.md#stylingseparaterollupitems) 和 [拆分任务](guides/split-tasks.md#yangshi) 设置样式
    - 可[隐藏项目任务中的所有 rollup 项](guides/milestones.md#hiding-tasks-and-milestones)
...
- 关键路径、时差与自动调度的改进:
    - 关键路径、时差和自动调度算法现在可利用任务进度:
     ...
     - [总时差](guides/critical-path.md#gettingfreeandtotalslack) 现在可为项目计算
     - 关键路径计算性能大幅提升
...
- 导出 API 已包含在 [gantt.plugins](guides/extensions-list.md#export-service) 中，无需额外添加 JS 文件。详情请查阅 [迁移指南](migration.md#71---80)
...
- 修复了使用分钟 [duration_unit](api/config/duration_unit.md) 和自定义 [工作时间](guides/working-time.md#global-settings) 设置时的 [工期计算](guides/working-time.md) 问题
- 修复了时差计算的各种问题
- 修复了启用 [时差计算](guides/critical-path.md#gettingfreeandtotalslack) 时数据加载出现的脚本错误
- [setWorkTime](api/method/setworktime.md) 方法现在支持为 customWeeks 内的日期设置规则
- 修复了智能渲染导致甘特图中出现空白的问题
- 修复了在某些行具有 [自定义高度](guides/resizing-rows.md#setting-the-row-height) 时，[表格中的行垂直重新排序](guides/reordering-tasks.md) 工作不正确的问题
...
- 修正了 [内联编辑器](guides/inline-editing.md#custominlineeditor) 接口 `is_valid` 函数的参数。该函数现在接收列对象
...
- 修复了 [资源面板](guides/resource-management.md#resourceviewpanel) 中行的 `odd` CSS 类顺序不正确的问题
...
- 修复了在不同 [时间刻度](guides/configuring-time-scale.md) 下，[S-Curve Overlay](guides/baselines.md#extra-overlay-for-the-chart) 显示异常的问题
- 修复了启用 [grid_elastic_columns](api/config/grid_elastic_columns.md) 属性时，[网格列调整大小](guides/specifying-columns.md#resizing) 的问题
- 修复了使用 [键盘导航](guides/keyboard-navigation.md) 删除任务后，甘特图垂直滚动条位置被重置的问题
- 修复了 [treeDatastore.move()](api/other/treedatastore.md) 方法的异常行为
- 修复了 [gantt.parse()](api/method/parse.md) 方法的问题，现在数据集中的 [额外集合](guides/supported-data-formats.md#jsonwithcollections) 可通过 [gantt.serverList()](api/method/serverlist.md) 方法获取
...
- 修复了使用单元格编辑器编辑资源值时，[资源面板](guides/resource-management.md#resourceviewpanel) 消失的问题
- 修复了当 [Gantt 布局](guides/layout-config.md) 包含 ["resourceGrid"/"resourceTimeline"](guides/resource-management.md#resourceviewpanel) 视图但未包含 "grid"/"timeline" 视图时发生的脚本错误
- 修复了当 Gantt 布局包含 [资源面板](guides/resource-management.md#resourceviewpanel) 时，[autosize](api/config/autosize.md) 配置项的异常行为
...
- 修复了激活 [click_drag](guides/extensions-list.md#advanced-drag-n-drop) 和 [drag_timeline](guides/extensions-list.md#drag-timeline) 扩展时，[onBeforeDrag](guides/advanced-dnd.md) 事件未能阻止默认操作的问题
- 修复了为 [资源分配存储](guides/resource-management.md#managingresourceassignments) 调用 [changeId()](api/other/datastore.md) 方法时抛出的脚本错误
- 默认 [Gantt 布局](guides/layout-config.md#default-layout) 的滚动条现在无需固定 **scrollVer**/**scrollHor** 名称即可正常工作
...
- 修复了如果 [项目任务](guides/task-types.md#project-tasks) 没有子任务且指定了 `start_date` 参数时，Gantt 未渲染任务的问题
...
- 修复了在使用 [后向计划](guides/auto-scheduling.md#backwardscheduling) 并将 [project_end](api/config/project_end.md) 设为非工作时间时，里程碑 `end_date` 计算不正确的问题
- 修复了如果 HTML 元素显示在 gantt 上方，任务重新排序异常的问题
- 修复了 [unsetWorkTime()](api/method/unsetworktime.md) 方法在日历中移除日期/天配置后未能立即生效的问题
- 修复了 [clearAll()](api/method/clearall.md) 方法在启用 [multiselect](guides/extensions-list.md#multitaskselection) 扩展时未能清除选中任务的问题
...
- 修复了 [自动排程](guides/auto-scheduling.md) 中，[约束类型](guides/auto-scheduling.md#timeconstraintsfortasks) 在更改任务持续时间后从 "ASAP" 变为 "SNET" 的问题
- 修复了启用 [schedule_from_end](api/config/schedule_from_end.md) 时，通过内联编辑器更改开始和结束日期后，[后向计划](guides/auto-scheduling.md#backwardscheduling) 异常的问题
- 现在可以在只读模式下为只读任务打开 lightbox [只读任务的 lightbox](guides/readonly-mode.md#readonlymodeforspecifictaskslinks)
- 现在无法通过 lightbox 编辑只读任务
- 修复了在 [只读模式下可编辑任务](guides/readonly-mode.md#readonlymodefortheentiregantt) 无法打开 lightbox 的问题（出现在 v6.3.1）
...
- 修复了在为无链接的任务设置 [ASAP 约束](guides/auto-scheduling.md#timeconstraintsfortasks) 并启用 [strict mode](api/config/auto_scheduling_strict.md) 后，[onBeforeTaskAutoSchedule](api/event/onbeforeautoschedule.md) 事件工作不正确的问题
- 修复了在 Next.js 项目中运行 Gantt 压缩版本时出现的错误
- 修复了在空容器中初始化 [gantt 实例](guides/multiple-gantts.md#gantt-instance-configuration) 后，Gantt 宽度变化的问题
...
- 修复了在启用 [资源直方图](guides/resource-management.md#resourceviewpanel) 和 [fit_tasks](api/config/fit_tasks.md) 配置时，[gantt.groupBy](guides/grouping.md) 方法抛出脚本错误的问题
...
- 修复了 [资源分配表单](guides/resources.md) 异常，导致 gantt 用自动生成的值替换用户自定义的 [资源分配](guides/resource-management.md#managingresourceassignments) id 的问题
...
- 提升了[总浮动](guides/critical-path.md#gettingfreeandtotalslack)计算的性能
- 修复了[Material](guides/skins.md#materialskin)皮肤下[lightbox](guides/edit-form.md)的样式问题
...
- 修复了将[inherit_calendar](guides/working-time.md#assigningcalendartoproject)配置与[gantt.groupBy](guides/grouping.md)方法一起使用时产生的脚本错误
...
- 修复了[列大小调整器](guides/specifying-columns.md#resizing)与[列重新排序](api/config/reorder_grid_columns.md)冲突导致的列大小调整异常
...
- 修复了在[gantt 重新初始化](api/method/init.md)并[移除所有列](guides/specifying-columns.md#overview)后，网格显示不正确的问题
- 修复了资源面板配置会覆盖 Vue.js 应用中[gantt 主配置](guides/common-configuration.md#ganttconfigobject)的问题
- 现在可以通过修改传递给[资源布局](guides/layout-config.md#configs-and-templates-of-views)的配置对象，动态修改[资源面板](guides/resource-management.md#resourceviewpanel)的配置
...
- 修复了在[Resource histogram](guides/resource-management.md#resourceviewpanel)滚动时，当 [resource_render_empty_cells](api/config/resource_render_empty_cells.md) 设置为 false 且 [smart_rendering](api/config/smart_rendering.md) 启用时抛出的脚本错误
- 修复了[Inline Editors](guides/inline-editors-ext.md)模块的 `editNextRow` 和 `editPrevRow` 方法工作不正确的问题
- 修复了[Quick Info](guides/extensions-list.md#quick-info) 弹窗点击网格"添加"按钮后仍然显示的问题
- 修复了[ASAP 约束](guides/auto-scheduling.md#timeconstraintsfortasks)未能将任务移动到项目最早日期的问题
- 修复了[Inline Editors](guides/inline-editors-ext.md)无法通过内联编辑器编辑[约束](guides/auto-scheduling.md#timeconstraintsfortasks)的问题
- 修复了[键盘导航](guides/keyboard-navigation.md)的"滚动到可见"逻辑导致任务条已可见时仍然滚动的问题
- 修复了启用[click_drag](guides/extensions-list.md#advanced-drag-n-drop)扩展时鼠标移出容器产生的脚本错误
...
- 显著提升了[资源面板](guides/resource-management.md#resourceviewpanel)的性能
...
- 修复了[click_drag](guides/extensions-list.md#advanced-drag-n-drop)插件的回归问题
...
- 优化了大量[任务日历](guides/working-time.md#assigningcalendartotask)场景下的性能
- 优化了[资源分配计算](guides/resource-management.md#managingresourceassignments)在[batchUpdate](api/method/batchupdate.md)和[autoScheduling](guides/auto-scheduling.md)过程中的性能
...
- 通过新的[gantt.config.process_resource_assignments](api/config/process_resource_assignments.md)和[gantt.updateTaskAssignments()](api/method/updatetaskassignments.md) Gantt API，[管理资源分配](guides/resource-management.md#managingresourceassignments)
- [汇总任务与里程碑](guides/milestones.md#rolluptasksandmilestones)
- [可在时间轴区域隐藏任务条与里程碑](guides/milestones.md#hiding-tasks-and-milestones)
- [可为不同时间段设置不同工作时间](guides/working-time.md#rules_for_periods)
- [可为网格中的单独行设置高度](guides/resizing-rows.md#setting-the-row-height)
...
- 网格单元格渲染到 DOM 时新增[onrender](guides/specifying-columns.md#modifyingcellsafterrendering)回调
...
- 修复了[Inline Editors](guides/inline-editing.md)在通过[hide:true](guides/specifying-columns.md#visibility)属性隐藏列后，无法在该列之后的单元格打开编辑器的问题
...
- 修复了使用 `fetchTasks` 选项时，[资源直方图](guides/resource-management.md#resourceviewpanel)初始内部高度异常的问题
- 修复了[前置任务编辑器](guides/inline-editing.md#types-of-editors)编辑值导致删除现有关联的问题
...
- Layout 的 [HTML 视图](guides/layout-config.md#htmlzuoweineibushitu) 现在支持外部[滚动条](guides/layout-config.md#scrollbar)
...
- 修复了在显示[弹出消息](guides/message-boxes.md#basic-popup-message)时销毁 gantt（[destroyed](api/method/destructor.md)）会抛出脚本错误的问题
...
- 修复了用户点击 [Quick Info](guides/extensions-list.md#quick-info) 弹窗时 Gantt 自动关闭弹窗的问题
...
- 修复了 [垂直调整器](guides/layout-config.md#default-layout) 的异常表现（v7.0.9 中出现的回归问题）
- 防止在 Android Chrome 上任务垂直重排时出现意外页面刷新（下拉刷新）
- 修复了在移动端 Firefox 上创建链接时触发的脚本错误
- 修复了在启用 [multiselect](api/config/multiselect.md) 配置但未激活 [multiselect](guides/extensions-list.md#multitaskselection) 插件时任务选择异常的问题
...
- 修复了在使用 [FF 和 SS 链接](api/config/links.md) 及源/目标任务使用[不同工作日历](guides/working-time.md#multipleworktimecalendars)时，[自动调度](guides/auto-scheduling.md)的异常表现
...
- 修复了同时使用[键盘导航](guides/extensions-list.md#keyboardnavigation)和[Quick Info](guides/extensions-list.md#quick-info) 扩展时，[gantt.destructor](api/method/destructor.md) 抛出的脚本错误
...
- 减少了[资源直方图](guides/resource-management.md#resourceviewpanel)的多余重绘次数
...
- [`Gantt.getGanttInstance`](guides/multiple-gantts.md#gantt-instance-configuration) 配置对象中可指定工作日历
...
- 修复了 [dataProcessor.setTransactionMode](guides/server-side.md#technique) 的第二个参数在第一个参数为对象时被忽略的问题
...
- 修复了在创建[多个 Gantt 实例](guides/multiple-gantts.md)时，[click_drag](guides/extensions-list.md#advanced-drag-n-drop) 和 [drag_timeline](guides/extensions-list.md#drag-timeline) 扩展的回归问题
- 修复了从 [dataProcessor 路由函数](guides/server-side.md#customrouting)返回错误状态后，任务行元素的 css 类不正确的问题
...
- [dataProcessor custom router](guides/server-side.md#customrouting) 现在能正确处理被拒绝的 Promise
- 修复了 [smart rendering](guides/performance.md#smart-rendering) 的回归问题，该问题导致部分链接不可见
...
- 支持[以分钟为单位设置工作时间](guides/working-time.md#global-settings)

### 更新

- 本地化文件已从包中移除，新增了适用于 Gantt 图本地化的 [新 API](api/other/i18n.md)
- 所有扩展现在需通过 [plugins](api/method/plugins.md) 方法激活
- `Gantt.getGanttInstance` 现在在创建新 Gantt 实例时[可接收配置对象](guides/multiple-gantts.md#gantt-instance-configuration)
...
- 修复了当 [task_height](api/config/task_height.md) 小于 [row_height](api/config/row_height.md) 时，[smart rendering](guides/performance.md#smart-rendering) 工作不正确的问题
...
- 修复了关闭 [smart rendering](guides/performance.md#smart-rendering) 后 [资源负载图](guides/resource-management.md#resourceviewpanel) 崩溃的问题
...
- 修复了同时启用 [scrollable](guides/specifying-columns.md#horizontal-scrollbar) 属性和 [autofit](api/config/autofit.md) 配置时，网格单元格对齐行为异常的问题
- 现在已阻止在时间轴中的任务与网格中的 [placeholder](api/config/placeholder_task.md) 创建链接
- 修复了 [自动调度扩展](guides/auto-scheduling.md) 的 bug，该 bug 导致当任务存在[约束类型 (SNET/FNET/SNLT/FNLT)](guides/auto-scheduling.md#timeconstraintsfortasks) 且未指定日期或日期无效时，Gantt 卡死
...
- 修复了[网格调整大小](guides/specifying-columns.md#resizing)行为异常导致时间轴不可用的问题
...
- 为 gantt 内部元素添加了缺失的 [WAI-ARIA](guides/accessibility.md#wai-aria-attributes) 属性
- 修复了 [min_duration](api/config/min_duration.md) 配置项工作不正确的问题
- 修复了 [link formatters](guides/formatters-ext.md#linkformatter) 在自定义 [gantt 实例](guides/multiple-gantts.md) 下工作不正确的问题
...
- 修复了 [smart rendering](api/method/addtasklayer.md#smart-rendering-for-custom-layers) 回归问题，该问题导致部分链接未被渲染
...
- 修复了 [drag-timeline 扩展](guides/extensions-list.md#drag-timeline) 会重置 [readonly config](guides/readonly-mode.md) 值的 bug
...
- [支持以小数单位指定任务工期](guides/working-time.md#taskdurationindecimalformat)
- [支持通过鼠标点击并拖动滚动时间轴](guides/extensions-list.md#drag-timeline)
- [支持水平拖拽多个任务](guides/multiselection.md#multitaskselectionanddragndrop)


### 更新

- 支持在[时间刻度](guides/configuring-time-scale.md#range)的显式 [start_date](api/config/start_date.md) 和 [end_date](api/config/end_date.md) 范围之外[显示任务](api/config/show_tasks_outside_timescale.md)
- 新增 [task_end_date](api/template/task_end_date.md) 模板，用于格式化任务的结束日期
- 支持将自定义操作添加到 [撤销](guides/undo-redo.md#undoingredoingchangesmadefromcode) 堆栈中
- 支持将自定义图层连接到[智能渲染](api/method/addtasklayer.md#smart-rendering-for-custom-layers)
...
- Roboto 字体已从 [Material 皮肤](guides/skins.md#materialskin)中移除，需手动引入

### 修复

- 修复关闭[智能渲染](guides/performance.md#smart-rendering)时[资源直方图](guides/resource-management.md#resourceviewpanel)崩溃的问题
- 修复与 r.js 压缩器的兼容性问题
- 修复[键盘导航](guides/keyboard-navigation.md)与[内联编辑器](guides/inline-editing.md)之间的各种冲突
- 修复从[自定义路由](guides/server-side.md#customrouting)顺序修改任务和链接时 [DataProcessor](guides/server-side.md#customrouting) 状态不正确的问题
- 现在在调用[自定义路由](guides/server-side.md#customrouting)的 **delete** 时也会传递正确的任务/链接数据对象
...
- 修复在[复杂布局](guides/resource-management.md#resourceviewpanel)中[带有水平滚动的表格](guides/specifying-columns.md#horizontal-scrollbar)垂直调整大小的问题
- 修复当[刻度步长](guides/configuring-time-scale.md#timestep)大于 1 时，[资源直方图](guides/resource-management.md#resourceviewpanel)工作异常的问题
...
- 修复 [QuickInfo 弹窗](guides/extensions-list.md#quick-info) 未显示在[未排期任务](guides/unscheduled-tasks.md)上的问题
...
- 修复[资源视图](guides/resource-management.md#resourceviewpanel)中[智能渲染](guides/performance.md#smart-rendering)异常的问题
...
- 修复在 IE11 和 MS Edge 浏览器中 [约束控制](guides/auto-scheduling.md#timeconstraintsfortasks)异常的问题
...
- 修复 [Material 皮肤](guides/skins.md#materialskin)下 [static_background](api/config/static_background.md) 显示异常的问题
...
- 修复[标记](guides/markers.md)开始日期早于[时间刻度](guides/configuring-time-scale.md#range)最小日期时未显示的问题
...
- 支持[展开/收起拆分任务](guides/split-tasks.md#expandingcollapsingsplittasks)（PRO）
...
- 修复[垂直拖放](guides/reordering-tasks.md)后，启用[智能渲染](guides/performance.md#smart-rendering)和[键盘导航](guides/keyboard-navigation.md)扩展时脚本错误
- 修复当部分表格列[隐藏](guides/specifying-columns.md#visibility)时，用户尝试用 `Tab` 键切换[内联编辑器](guides/inline-editing.md)导致的异常
- 修复阻止[lightbox](guides/edit-form.md)和[内联编辑器](guides/inline-editing.md)覆盖[约束日期](guides/auto-scheduling.md#timeconstraintsfortasks)时的异常行为
...
- 修复在第二次调用 [init](api/method/init.md) 后，[QuickInfo 弹窗](guides/extensions-list.md#quick-info)的[点击处理器](api/config/quickinfo_buttons.md)无效的问题
- 修复设置 [show_chart](api/config/show_chart.md) 为 false 时，[QuickInfo 弹窗](guides/extensions-list.md#quick-info)未显示的问题
- 修复[垂直拖放](guides/reordering-tasks.md)后，[dataProcessor 路由](guides/server-side.md#customrouting)的 `action` 参数不正确的问题
...
- 修复在 [keyboard_navigation_cells](api/config/keyboard_navigation_cells.md) 模式下，表格包含[隐藏列](guides/specifying-columns.md#visibility)时，[内联编辑器](guides/inline-editing.md)异常
...
- 在 [智能渲染](guides/performance.md#smart-rendering) 模式下渲染大量任务列表时小幅提升性能
...
- 修复从 JSON 数据解析 [约束日期](guides/auto-scheduling.md#timeconstraintsfortasks) 时的脚本错误
...
- 修复当启用 [资源面板](guides/resource-management.md#resourceviewpanel) 时，调用 [gantt.destructor](api/method/destructor.md) 引起的脚本错误
...
- 修复新建数据存储覆盖原有数据存储后 [资源面板](guides/resource-management.md#resourceviewpanel) 工作不正确的问题
...
- 修复 [gantt.destructor](api/method/destructor.md) 与 [资源面板](guides/resource-management.md#resourceviewpanel) 一起使用时的脚本错误
...
- [为甘特图添加覆盖层的能力](guides/baselines.md#extra-overlay-for-the-chart)（PRO）
- [任务时间约束](guides/auto-scheduling.md#timeconstraintsfortasks)（PRO）
- [反向排程](guides/auto-scheduling.md#backwardscheduling)（PRO）
- TypeScript 类型定义已加入包内

### 更新

- 可为 dhtmlxGantt [所有元素创建提示](guides/tooltips.md#tooltipsfordifferentelements)
- [dataProcessor 路由选项](guides/server-side.md#customrouting)
- [项目级工作日历](guides/working-time.md#assigningcalendartoproject)（PRO）
...
- 减少了 [资源图表](guides/resource-management.md#resourceviewpanel) 多余的重绘次数
- 修复删除任务后 [资源图表](guides/resource-management.md#resourceviewpanel) 的脚本错误
...
- [为任务分配多个资源](guides/resource-management.md#managingresourceassignments)（PRO 版本）
- [按多个资源分组任务](guides/resource-management.md#balancingresourceload)（PRO 版本）
- [资源直方图](guides/resource-management.md#resourceviewpanel)，用于展示资源负载图之外的资源分布（PRO 版本）
- 在 [关键路径计算](guides/critical-path.md) 时 [获取任务的自由/总缓冲时间](guides/critical-path.md#gettingfreeandtotalslack)（PRO 版本）
- [从 Excel 导入项目](guides/excel.md#importfromexcel)
...
- 更新了 [键盘导航](guides/keyboard-navigation.md#existingshortcuts)
...
- [网格横向滚动](guides/specifying-columns.md#horizontal-scrollbar) 及 [其他布局改进](guides/layout-config.md)
- [Gantt 与 DataProcessor 实例的析构函数](guides/multiple-gantts.md#destructorofganttanddataprocessorinstances)

### 配置

- [可为网格列设置最小/最大宽度](guides/specifying-columns.md#width)
- [可拖动项目及其子任务](guides/dnd.md#draggingprojectswithsubtasks)（PRO 版本）
- [导出方法的扩展参数](guides/export-common.md)

### API

- [多任务选择](guides/multiselection.md) 扩展的 [API 事件](guides/multiselection.md#apievents) 已更新
...
- 新的 ["Material" 皮肤](guides/skins.md#materialskin)
...
- [任务和资源级别的工作时间日历](guides/working-time.md#multipleworktimecalendars)
- [WBS 代码（大纲编号）计算](guides/specifying-columns.md#wbscode)
- [拖放操作的自动滚动](guides/dnd.md#autoscrollduringtasksdragging)
- [新增波斯语（法尔西语）本地化](guides/localization.md#predefinedlocales)

### 配置

- 新增用于键盘导航快捷键的 [getter 函数](api/method/getshortcuthandler.md)
- 新增用于级联删除嵌套任务和链接的 [配置项](api/config/cascade_delete.md)
- 新增 [按住 Shift 并滚动鼠标滚轮可水平滚动时间轴](guides/keyboard-navigation.md#builtinshortcutforhorizontaltimelinescrolling) 的功能
- 更新德语和意大利语本地化
...
- [WAI-ARIA 支持](guides/accessibility.md#wai-aria-attributes)
...
- 新增 [克罗地亚语本地化](guides/localization.md#predefinedlocales)
- 更新 [土耳其语本地化](guides/localization.md#predefinedlocales)
...
- 针对大型数据集的 [智能渲染](guides/performance.md#smart-rendering)
- [撤销/重做](guides/undo-redo.md) 扩展

### 配置

- [关键路径计算](guides/critical-path.md) -- 支持 [链接的滞后/提前时间](guides/auto-scheduling.md#settinglagandleadtimesbetweentasks)（PRO 版本）
- 更新西班牙语和中文 [本地化](guides/localization.md#predefinedlocales)
...
- [反向计划](guides/loading.md#loadingtaskdates)

### 配置

- 初步支持 [内容安全策略](guides/content-security-policy.md)
- 可指定 [每列的表格排序](guides/sorting.md#percolumngridsorting) 设置
...
- [用于管理任务重新排序时可用目标位置的事件](guides/reordering-tasks.md#denyingdroppingtospecificpositions)
- [用于管理加载流程的事件](guides/loading.md#eventsflow)
- 新增示例、方法和事件
...
- [里程碑](guides/milestones.md) 和 [项目](guides/task-types.md#project-tasks) 支持（PRO 版本）
...
- [所有文本元素都可通过模板定义](guides/common-configuration.md#gantttemplatesobject)
- [所有日期字符串均可配置](guides/common-configuration.md#ganttconfigobject)
