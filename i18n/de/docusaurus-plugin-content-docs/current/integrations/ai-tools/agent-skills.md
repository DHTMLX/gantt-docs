...
Beinhaltet die Integration von `@dhtmlx/trial-vue-gantt` und `@dhx/vue-gantt` in Vue 3-Anwendungen - dieselben wrapper-spezifischen Muster, ergänzt um Events, `@ready`, Refs und Composables.

Überprüfen Sie die Regeln im [GitHub-Repository](https://github.com/DHTMLX/skills/tree/main/dhtmlx-vue-gantt).
...
Klonen oder herunterladen Sie das [DHTMLX/skills](https://github.com/DHTMLX/skills)-Repository und kopieren Sie den jeweiligen Skill-Ordner (`dhtmlx-js-gantt`, `dhtmlx-react-gantt`, `dhtmlx-angular-gantt` oder `dhtmlx-vue-gantt`) in das Skills-Verzeichnis Ihres Projekts (z. B. `.claude/skills/` für Claude Code, `.cursor/skills/` für Cursor).
