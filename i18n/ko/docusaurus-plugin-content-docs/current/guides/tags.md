...
일부 속성은 MPP 파일에서만 수신되며 XML 파일에는 저장되지 않으므로 주의하십시오. 따라서 MPP 파일을 XML 형식으로 저장한 다음 MS Project에서 열면 해당 속성은 XML 파일에서도 존재하지 않습니다. 예를 들어, 모든 OutlineCode 속성은 이 방식으로 동작합니다: 목록에서 값을 선택할 수 있어도 XML 파일에 저장된 값은 비어 있습니다.

일부 속성은 다른 속성에 의해 계산되므로 필요한 속성이 가져오기 동안 지정되지 않으면 값이 다르게 나타납니다. 보다 편리하게 만들기 위해 내보내기 모듈 측에서 계산을 추가했습니다. 따라서 XML 파일을 가져올 때 언급된 속성들은 값을 가지지만, XML 파일을 내보내고 MS Project에서 열었을 때의 결과와 다를 수 있습니다.

## 자원 속성

:::info
일부 속성은 MPP 파일에서만 수신될 수 있으며, 다른 속성은 XML 파일에서만 수신될 수 있습니다.

일부 속성은 다른 속성에 의해 계산되므로 가져오기 동안 필요한 속성이 지정되지 않으면 값이 다르게 나타납니다.
:::
