...
- PDF/PNG 렌더링 엔진을 Puppeteer로 마이그레이션
- 다중 페이지 PDF 내보내기에서 각 페이지에 표시되던 헤더와 푸터가 이제 페이지 콘텐츠 위에 표시됩니다
- PDF 및 PNG 내보내기에서 Scheduler에 대해 올바른 DPI 스케일 설정
- Docker 이미지 크기를 축소했습니다
