...
This tutorial describes how to build a [React Gantt](web/react.md) chart that synchronizes task and link data across multiple clients 
...
{{note Replace the `YOUR_API_KEY` placeholder with your actual Firebase project credentials.}}
...
To begin with, set up the core Gantt component with [**React state** for tasks and links](web/react.md#bindingdata) with the following configuration: 
...
Since Firestore stores dates as strings, we need to convert them back to `Date` objects ([parse_date](api/gantt_parse_date_template.md)) and correctly format them for storage ([format_date](api/gantt_format_date_template.md)).
