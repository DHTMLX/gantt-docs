...
gantt.attachEvent("onGanttScroll", (left, top) => {
	const leftDate = gantt.dateFromPos(left);
	const rightDate = gantt.dateFromPos(left + gantt.$task.offsetWidth);

	gantt.config.start_date ||= gantt.getState().min_date;
	gantt.config.end_date ||= gantt.getState().max_date;

	const minAllowedDate = gantt.date.add(gantt.config.start_date, 1, "day");
	const maxAllowedDate = gantt.date.add(gantt.config.end_date, -2, "day");
...
gantt.attachEvent("onMouseMove", (id, e) => {
	if (!gantt.getState().drag_id && e.buttons === 1) {
		...
		const leftDate = gantt.dateFromPos(scrollState.x);
		const rightDate = gantt.dateFromPos(scrollState.x + gantt.$task.offsetWidth-1);

		if (leftDate && +leftDate <= +gantt.config.start_date) {
		...
		if (
			rightDate &&
			+gantt.config.end_date < +gantt.date.add(rightDate, 1, "day")
		) {
...
gantt.config.start_date = new Date(2026, 2, 28);
gantt.config.end_date = new Date(2026, 3, 10);
gantt.render();
	...
	*/
	{
		...
		template: task => `<button onclick="addTask(${task.id})"></button>`
	},
];
~~~
...
~~~js
gantt.date.custom_scale_start = date => date;

gantt.date.add_custom_scale = (date, inc) => {
...
gantt.date.five_days_start = (date) => date;

gantt.date.add_five_days = (date, inc) => {
...
gantt.ignore_time = (date) => date.getDay() == 0 || date.getDay() == 6;
~~~
...
~~~js
gantt.date.custom_week_start = (date) => date;

gantt.date.add_custom_week = (date, inc) => {
	const yearStart = new Date(date);
	gantt.date.year_start(yearStart);
	const weekNumber = Math.round(gantt.calculateDuration(yearStart, date) / 7);

	const nextWeek = gantt.date.add(yearStart, weekNumber + 1, "week");
	if (nextWeek.getYear() != date.getYear()) {
		gantt.date.year_start(nextWeek);
	}
	return nextWeek;
};

const customWeekTemplate = (date) => {
	const yearStart = gantt.date.year_start(new Date(date));
	const weekNumber = Math.round(gantt.calculateDuration(yearStart, date) / 7) + 1;
	return "Week:" + weekNumber;
};

gantt.config.scales = [
	{ unit: "custom_week", step: 1, template: customWeekTemplate },
	...
	*/
	{
		name: "clone", label: "clone", width: 44, template: function (task) {
			return `<input type="button" value="V" onclick="cloneTask(${task.id})">`;
		}
	}
];
~~~
...
~~~js
let childLinks;
let cloneOriginalIdsTable;

function obtainLinkIds(id) {
	const task = gantt.getTask(id);
	const sourceLinks = task.$source;
	for (let i = 0; i < sourceLinks.length; i++) {
		childLinks.push(sourceLinks[i]);
	}
}

function createCloneOriginalIdsTable(originalId, cloneId) {
	cloneOriginalIdsTable[originalId] = cloneId;
}

function cloneChildLinks() {
	for (let i = 0; i < childLinks.length; i++) {
		const link = gantt.getLink(childLinks[i]);
		if (
			cloneOriginalIdsTable[link.source] &&
			cloneOriginalIdsTable[link.target]
		) {
			const cloneLink = {
				id: gantt.uid(),
				source: cloneOriginalIdsTable[link.source],
				target: cloneOriginalIdsTable[link.target],
				type: link.type
			};
			gantt.addLink(cloneLink);
		}
	}
}

function cloneChildren(id, newParent) {
	const children = gantt.getChildren(id);
	for (let i = 0; i < children.length; i++) {
		const childOriginal = gantt.getTask(children[i]);
		const childClone = gantt.copy(childOriginal);
		childClone.id = gantt.uid();
		childClone.parent = newParent;
		gantt.addTask(childClone, childClone.parent, childClone.$index);

		obtainLinkIds(childOriginal.id);
		createCloneOriginalIdsTable(childOriginal.id, childClone.id);

		if (gantt.hasChild(childOriginal.id)) {
			cloneChildren(childOriginal.id, childClone.id);
		}
	}
}
...
function cloneTask(id) {
	...
	cloneOriginalIdsTable = {};
	createCloneOriginalIdsTable(task.id, clone.id);

	if (gantt.hasChild(id)) {
		cloneChildren(id, clone.id);
	}
...
gantt.config.columns = [
	...
	*/
	{
		name: "clone", label: "clone", width: 44, template: (task) => {
...
gantt.ext.keyboardNavigation.addShortcut("ctrl+c", (e) => {
	tasksToCopy = [];
	gantt.eachSelectedTask((taskId) => {
		tasksToCopy.push(taskId);
	});
}, "taskRow");

gantt.ext.keyboardNavigation.addShortcut("ctrl+v", (e) => {
	const newParent = gantt.getSelectedId();
	for (let i = 0; i < tasksToCopy.length; i++) {
		const task = gantt.copy(gantt.getTask(tasksToCopy[i]));
		task.id = `${Date.now()}+${Math.floor(Math.random() * 10)}`;
...
function parentProgress(id) {
	gantt.eachParent((task) => {
		...
		task.progress = children.length ? childProgress / children.length / 100 : 0;
	}, id);
...
~~~js
gantt.event(gantt.$root, "click", (e) => {
