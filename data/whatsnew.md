...
- Reduced the number redundant repaints of the [resource diagram](desktop/resource_management.md#resourceviewpanel)
- Fixed script error from the [resource diagram](desktop/resource_management.md#resourceviewpanel) after deleting a task
- Fixed script error from [fullscreen extension](desktop/fullscreen_mode.md) after exiting fullscreen mode on `esc` key
- Fixed the incorrect state of links drag and drop when dragging link between multiple charts on the page, creating links between gantts is not supported
